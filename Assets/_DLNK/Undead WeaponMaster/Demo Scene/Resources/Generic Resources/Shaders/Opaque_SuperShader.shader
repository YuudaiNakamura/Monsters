...
/////// Normals:
                ...
                float3 normalLocal = lerp(UnpackNormal(tex2D(_Normal,TRANSFORM_TEX(node_309.rg, _Normal))).rgb,float3(0,0,1),_NormalSmooth);
...
/////// Diffuse:
                ...
                float node_18 = 0.0;
                float3 w = lerp( node_18, (tex2D(_TransMask,TRANSFORM_TEX(node_309.rg, _TransMask)).a*_TransPower*_TransColor.rgb), _Translucency )*0.5; // Light wrapping
                float3 NdotLWrap = NdotL * ( 1.0 - w );
                float3 forwardLight = max(float3(0.0,0.0,0.0), NdotLWrap + w );
                float3 diffuse = forwardLight * attenColor + UNITY_LIGHTMODEL_AMBIENT.rgb;
////// Emissive:
                float3 node_65 = (_ReflectionTint.rgb*(texCUBE(_ReflectionMap,viewReflectDirection).rgb*tex2D(_ReflectionMask,TRANSFORM_TEX(node_309.rg, _ReflectionMask)).a)*_ReflectionPower);
                float3 emissive = (node_65*_ReflectionEmission);
///////// Gloss:
                float node_20 = 1.0;
                float gloss = (lerp( node_20, tex2D(_Gloss,TRANSFORM_TEX(node_309.rg, _Gloss)).a, _CustomGloss )*_Shinniness);
                float specPow = exp2( gloss * 10.0+1.0);
////// Specular:
                NdotL = max(0.0, NdotL);
                float node_24 = lerp( node_20, max(min(saturate(( _AOLevel > 0.5 ? (1.0-(1.0-2.0*(_AOLevel-0.5))*(1.0-lerp(tex2Dlod(_AmbientOclussion,float4(TRANSFORM_TEX(node_309.rg, _AmbientOclussion),0.0,_AODetail)).a,node_20,_AOBurn))) : (2.0*_AOLevel*lerp(tex2Dlod(_AmbientOclussion,float4(TRANSFORM_TEX(node_309.rg, _AmbientOclussion),0.0,_AODetail)).a,node_20,_AOBurn)) )),node_20),node_18), _AOEnable );
                float3 specularColor = (tex2D(_Specular,TRANSFORM_TEX(node_309.rg, _Specular)).a*_SpecularLevel*node_24*_SpecColor.rgb);
                float3 specular = (floor(attenuation) * _LightColor0.xyz) * pow(max(0,dot(halfDirection,normalDirection)),specPow) * specularColor;
                float3 finalColor = 0;
                float3 diffuseLight = diffuse;
                diffuseLight += (saturate(min(max(node_18,(pow(1.0-max(0,dot(normalDirection, viewDirection)),_RimFresnel)*_RimPower*_RimColor.rgb)),tex2D(_RimMask,TRANSFORM_TEX(node_309.rg, _RimMask)).a))+(UNITY_LIGHTMODEL_AMBIENT.rgb*_AmbientalPower)); // Diffuse Ambient Light
                float4 node_2 = tex2D(_Base,TRANSFORM_TEX(node_309.rg, _Base));
                float4 node_26 = tex2D(_MaskMap,TRANSFORM_TEX(node_309.rg, _MaskMap));
...
/// Final Color:
        ...
        Pass {
            ...
            fixed4 frag(VertexOutput i) : COLOR {
                ...
                float3 viewDirection = normalize(_WorldSpaceCameraPos.xyz - i.posWorld.xyz);
/////// Normals:
                float2 node_310 = i.uv0;
                float3 normalLocal = lerp(UnpackNormal(tex2D(_Normal,TRANSFORM_TEX(node_310.rg, _Normal))).rgb,float3(0,0,1),_NormalSmooth);
                float3 normalDirection =  normalize(mul( normalLocal, tangentTransform )); // Perturbed normals
                float3 viewReflectDirection = reflect( -viewDirection, normalDirection );
...
/////// Diffuse:
                float NdotL = dot( normalDirection, lightDirection );
                float node_18 = 0.0;
                float3 w = lerp( node_18, (tex2D(_TransMask,TRANSFORM_TEX(node_310.rg, _TransMask)).a*_TransPower*_TransColor.rgb), _Translucency )*0.5; // Light wrapping
                float3 NdotLWrap = NdotL * ( 1.0 - w );
                float3 forwardLight = max(float3(0.0,0.0,0.0), NdotLWrap + w );
                float3 diffuse = forwardLight * attenColor;
///////// Gloss:
                float node_20 = 1.0;
                float gloss = (lerp( node_20, tex2D(_Gloss,TRANSFORM_TEX(node_310.rg, _Gloss)).a, _CustomGloss )*_Shinniness);
                float specPow = exp2( gloss * 10.0+1.0);
////// Specular:
                NdotL = max(0.0, NdotL);
                float node_24 = lerp( node_20, max(min(saturate(( _AOLevel > 0.5 ? (1.0-(1.0-2.0*(_AOLevel-0.5))*(1.0-lerp(tex2Dlod(_AmbientOclussion,float4(TRANSFORM_TEX(node_310.rg, _AmbientOclussion),0.0,_AODetail)).a,node_20,_AOBurn))) : (2.0*_AOLevel*lerp(tex2Dlod(_AmbientOclussion,float4(TRANSFORM_TEX(node_310.rg, _AmbientOclussion),0.0,_AODetail)).a,node_20,_AOBurn)) )),node_20),node_18), _AOEnable );
                float3 specularColor = (tex2D(_Specular,TRANSFORM_TEX(node_310.rg, _Specular)).a*_SpecularLevel*node_24*_SpecColor.rgb);
                float3 specular = attenColor * pow(max(0,dot(halfDirection,normalDirection)),specPow) * specularColor;
                float3 finalColor = 0;
                float3 diffuseLight = diffuse;
                float4 node_2 = tex2D(_Base,TRANSFORM_TEX(node_310.rg, _Base));
                float4 node_26 = tex2D(_MaskMap,TRANSFORM_TEX(node_310.rg, _MaskMap));
                float3 node_65 = (_ReflectionTint.rgb*(texCUBE(_ReflectionMap,viewReflectDirection).rgb*tex2D(_ReflectionMask,TRANSFORM_TEX(node_310.rg, _ReflectionMask)).a)*_ReflectionPower);
                finalColor += diffuseLight * saturate(max((node_24*lerp( node_2.rgb, ((_ColorTint.rgb*node_26.a)+(node_2.rgb*(1.0 - node_26.a))), _ColorMask )),node_65));
                finalColor += specular;
/// Final Color:
