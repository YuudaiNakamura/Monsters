...
public class WarriorAnimation : MonoBehaviour {
	...
	public bool chain1;  //used to select which attack to chain to
	public bool chain2;  //used to select which attack to chain to
	public enum Warrior{Karate, Ninja, Brute, Sorceress, Knight, Mage, Archer, TwoHanded, Swordsman};
	...
	void Start() {
		...
		if(warrior == Warrior.Archer){ //sets the weight on any additional layers to 0
	...
	void Update(){
		if(!dead && !blockGui)  //if character isn't dead, blocking, or stunned (or in a move)
		{
			//Get input from controls
			float z = CrossPlatformInputManager.GetAxisRaw("Horizontal");
			float x = -(CrossPlatformInputManager.GetAxisRaw("Vertical"));
			...
			if (!isBlocking){  //if not blocking
				...
				if (Input.GetButtonDown("Fire0")){
					...
					if (warrior == Warrior.Brute)  //if character is Brute
						...
						//Instantiate(secondaryWeaponModel, this.gameObject.transform.position, Quaternion.identity);
				...
				if (Input.GetButtonDown("Fire1")){
					if(!canChain){ //used for characters who can chain attacks to chain to 2nd Attack
						...
						} else {  //if charater is not in air, do regular attack
						...
						if (warrior == Warrior.Brute || warrior == Warrior.Sorceress )  //if character is Brute or Sorceress
							...
							StartCoroutine (COStunPause(.9f));
						else if (warrior == Warrior.TwoHanded || warrior == Warrior.Swordsman)  //if character is 2Handed or Swordsman
							StartCoroutine (COStunPause(.9f));
						else if (warrior == Warrior.Swordsman)  //if character is Swordsman
						...
						if(chain1){
							...
							if(warrior == Warrior.Knight)
								...
								StartCoroutine(COChainWindow(.8f, 1.2f));
							else
	...
	void RotateTowardMovementDirection(){  //face character along input direction
		if(!dead){  //if character isn't dead
			if (!blockGui && !isBlocking && !isStunned){  //if character isn't stunned or blocking
	...
	void OnGUI (){
		if(!dead){  //if character isn't dead
			if(!blockGui && !isBlocking){  //if character is not blocking
			...
			else if(!inBlock){  //if not blocking
				if(!inBlock){
					if (GUI.Button (new Rect (30, 240, 100, 30), "Hit React")) {
						animator.SetTrigger("LightHitTrigger");
						StartCoroutine (COStunPause(2.2f));
					}
				}
			}
			if(!blockGui && !isBlocking){  //if blocking
				if (GUI.Button (new Rect (25, 115, 100, 30), "RangeAttack1")) {
					...
					if (warrior == Warrior.Brute)  //if character is Brute
						...
						StartCoroutine (COStunPause(1.8f));
					else if (warrior == Warrior.Ninja)  //if character is Ninj
						StartCoroutine (COStunPause(1.5f));
					else if (warrior == Warrior.Swordsman)  //if character is Ninj
						StartCoroutine (COStunPause(1f));
					else if (warrior == Warrior.Archer){  //if character is Archer
						StartCoroutine (COSetLayerWeight(.6f));
						StartCoroutine (COArcherArrow(.2f));
				...
				if (warrior == Warrior.Mage || warrior == Warrior.Ninja || warrior == Warrior.Knight || warrior == Warrior.Archer || warrior == Warrior.TwoHanded || warrior == Warrior.Swordsman){
					if (!dead && !weaponSheathed) {
						if (GUI.Button (new Rect (30, 310, 100, 30), "Sheath Weapon")) {
