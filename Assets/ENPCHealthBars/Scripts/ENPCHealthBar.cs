...
namespace SnazzlebotTools.ENPCHealthBars
{
    ...
    public class ENPCHealthBar : MonoBehaviour
    {
        ...
        void Awake()
        {
            ...
            InitLevelText();
            InitValueText();
			_faceCamera = GameObject.FindGameObjectWithTag ("MainCamera").GetComponent<Camera> ();
        }

        void Reset()
        {
            _healthBar = transform.FindChild("HealthBar");
            BarColor = Color.red;
        ...
        private void InitCanvas()
        {
            ...
            _healthBar = _healthBar ?? transform.FindChild("HealthBar");
            if (_healthBar == null)
            {
                _healthBar = new GameObject("HealthBar", typeof(Canvas)).transform;
                _healthBar.localScale = new Vector3(0.01f, 0.01f, 0);
        ...
        void Update()
        {
            ...
            {
                _healthBar.LookAt(_healthBar.position + FaceCamera.transform.rotation * Vector3.forward,FaceCamera.transform.rotation * Vector3.up);
				//_healthBar.LookAt(FaceCamera.transform.position);
            }
        }
